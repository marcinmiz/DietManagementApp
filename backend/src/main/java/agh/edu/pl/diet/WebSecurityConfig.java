...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeRequests()
                .antMatchers(HttpMethod.POST,"/resources/**", "/api/users/register", "/api/users/login", "/api/users/forgotPassword", "/api/users/checkTokenValidity", "/api/users/changePassword").permitAll()
    ...
    public void configure(final AuthenticationManagerBuilder auth) throws Exception {
        auth.inMemoryAuthentication()
                .withUser("admin1234").password("{noop}password").roles("ADMIN");
